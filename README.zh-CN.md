...
- 按遥控器设置键/菜单键，打开当前选中行的操作菜单。
- 设置键/菜单键弹窗同时支持“复制”和“剪切”；只有复制或剪切项目后才显示“粘贴”。
- 光标选中文件夹时会粘贴到该文件夹内部；选中文件时会粘贴到该文件所在目录。
...
1. 将光标停留在需要复制或剪切的文件/文件夹上，按遥控器设置键/菜单键。
2. 选择“复制”或“剪切”。
3. 进入目标位置并将光标停留在一个项目上：
   - 光标位于文件夹时，“粘贴”会把源项目放入该文件夹内部。
   - 光标位于文件时，“粘贴”会把源项目放在该文件的同级目录。
4. 再次按设置键/菜单键，然后选择“粘贴”。

没有复制或剪切项目时，菜单不会显示“粘贴”。复制项目可重复粘贴；剪切项目成功移动后会自动清空。文件夹支持递归复制；遇到同名项目时会自动添加 `(1)` 等后缀；应用会阻止把文件夹粘贴到自身或其子文件夹中。

## APK 选择

- `TVFileBox-v1.2.5-armeabi-v7a.apk`：32 位 ARM 老电视盒子。
- `TVFileBox-v1.2.5-arm64-v8a.apk`：64 位 ARM 设备。
- `TVFileBox-v1.2.5-universal.apk`：适合一般安装的通用版本。
